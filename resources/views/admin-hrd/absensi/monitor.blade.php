    ...
    <div class="card">
        ...
        <form method="GET" class="flex flex-wrap items-center gap-3">
            ...
            <select name="status_kehadiran" class="form-input w-44">
                ...
                <option value="terlambat" {{ request('status_kehadiran') === 'terlambat' ? 'selected' : '' }}>
                    Terlambat
                </option>
                <option value="alpha" {{ request('status_kehadiran') === 'alpha' ? 'selected' : '' }}>Alpha
                </option>
                ...
                <option value="sakit" {{ request('status_kehadiran') === 'sakit' ? 'selected' : '' }}>Sakit
                </option>
            </select>
            ...
            <a href="{{ route('admin.absensi.manual.index') }}" class="btn-secondary inline-flex items-center gap-2">
            ...
            <table class="w-full text-sm">
                <thead>
                    <tr class="border-b border-slate-100">
                        ...
                        <th class="text-left py-3 px-3 text-xs font-semibold text-slate-500 uppercase">Divisi & Jadwal
                        </th>
                ...
                <tbody>
                        ...
                        <tr class="table-row">
                            <td class="py-3 px-3">
                                <div class="flex items-center gap-3">
                                    <img src="{{ $row->karyawan->foto_url }}"
                                        class="w-9 h-9 rounded-xl object-cover ring-1 ring-slate-100 flex-shrink-0"
                                        alt="">
                                    <div>
                                        <div class="font-semibold text-slate-800">{{ $row->karyawan->nama_lengkap }}
                                        </div>
                            ...
                            <td class="py-3 px-3 text-xs">
                                <div class="font-semibold text-slate-700">
                                    {{ $row->karyawan->divisi?->nama_divisi ?? '-' }}</div>
                                @if ($row->karyawan->divisi)
                                    <div class="text-[11px] text-slate-400 font-mono">
                                        {{ $row->karyawan->divisi->jam_kerja_formatted }}</div>
                                @endif
                            </td>
                            <td class="py-3 px-3 font-mono text-xs font-bold text-slate-700">
                                {{ $row->waktu_masuk ? substr($row->waktu_masuk, 0, 5) : '-' }}</td>
                            <td class="py-3 px-3 font-mono text-xs font-bold text-slate-700">
                                {{ $row->waktu_keluar ? substr($row->waktu_keluar, 0, 5) : '-' }}</td>
                            <td class="py-3 px-3 text-xs font-semibold text-slate-600">
                                {{ $row->jam_kerja ? $row->jam_kerja . ' jam' : '-' }}</td>
                            <td class="py-3 px-3"><span
                                    class="badge badge-{{ $badge['color'] }}">{{ $badge['label'] }}</span></td>
                            <td class="py-3 px-3">
                                @if ($row->status_lokasi)
                                    <div>
                                        ...
                                        @if ($row->lat_masuk && $row->lng_masuk)
                                            <a href="https://www.google.com/maps?q={{ $row->lat_masuk }},{{ $row->lng_masuk }}"
                                                target="_blank" rel="noopener noreferrer"
                                                class="block text-[11px] text-sky-600 hover:text-sky-800 hover:underline font-semibold mt-1">
                                                📍 Lihat Maps ({{ round($row->lat_masuk, 4) }},
                                                {{ round($row->lng_masuk, 4) }})
                                            </a>
                                        @endif
                                    </div>
                                @else
                            ...
                            <td class="py-3 px-3">
                                @if ($row->status_face)
                                    <div class="flex items-center gap-2">
                                        <img src="{{ $row->karyawan->foto_enrollment_url ?? $row->karyawan->foto_url }}"
                                            class="w-8 h-8 rounded-lg object-cover ring-2 ring-emerald-400/50"
                                            title="Foto Terdaftar {{ $row->karyawan->nama_lengkap }}" alt="">
                                        <span
                                            class="badge {{ $row->status_face === 'berhasil' ? 'badge-green' : 'badge-red' }}">
                            ...
                            <td class="py-3 px-3 text-right">
                                <a href="{{ route('admin.absensi.detail', $row->id) }}"
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="py-10 text-center text-slate-400">Tidak ada data absensi untuk
                                tanggal tersebut.</td>
                        </tr>
                    @endforelse
                </tbody>
