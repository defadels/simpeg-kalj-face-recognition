        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('admin-hrd.karyawan.store') }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div><label class="form-label">Role *</label><select name="role" class="form-input" required><option value="karyawan" {{ old('role') === 'karyawan' ? 'selected' : '' }}>Karyawan</option><option value="manajer" {{ old('role') === 'manajer' ? 'selected' : '' }}>Manajer</option></select></div>
