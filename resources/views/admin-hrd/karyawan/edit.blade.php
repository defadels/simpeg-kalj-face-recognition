        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('admin-hrd.karyawan.update', $karyawan) }}" enctype="multipart/form-data" class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div><label class="form-label">NIP *</label><input type="text" name="nip" value="{{ old('nip', $karyawan->nip) }}" class="form-input" required>@error('nip')<p class="form-error">{{ $message }}</p>@enderror</div>
                    <div><label class="form-label">Nama Lengkap *</label><input type="text" name="nama_lengkap" value="{{ old('nama_lengkap', $karyawan->nama_lengkap) }}" class="form-input" required></div>
